$.bind('couleur', function(event) {
	var command = event.getCommand(),
		...
		args = event.getArgs();
		color = new ["rouge", "orange", "jaune", "vert", "cyan", "bleu", "violet", "rose"];
		
		// si un seul argument => verifier qu'il est dans le tableau
		if(args.getLengh(args) = 1)
		{
		
			// verifier que la coleur existe
			for(var i=0; i<color.length; i++)
				if (color.includes(args))
				{
...
(function() {
	var color = new Array("rouge", "orange", "jaune", "vert", "cyan", "bleu", "violet", "rose");
}
function lire1(tab)
{
